...
page. If Home Depot renders a reCAPTCHA/Turnstile challenge anywhere —
...
class CaptchaChallenge:
    ...
    def aws_waf_action(self) -> str:
        ...
        a sibling repo's site on 2026-09-17: challenge-action pages came back
...
def detect_aws_waf(html: str, page_url: str = "") -> Optional[CaptchaChallenge]:
    ...
    Measured on a SIBLING repo's site (www.a sibling repo.com) 2026-09-16:
    that site sits behind AWS
...
# longer matched the sibling a sibling repo repo's target)
# ---------------------------------------------------------------------------
#
...
# form is one example) was ever observed on Home Depot, and porting one
# on spec would be dead code with no fixture to test it against.
#
# What was measured FOR THIS REPO, rather than inherited from a sibling:
#
#   * 15 plain `curl` fetches of category pages, product pages and sitemaps
#     from a residential address in Moscow on 2026-09-17/18: HTTP 200 every
#     time, and 0 occurrences of every marker in
#     product_parser.BOT_CHALLENGE_MARKERS across the 14 served HTML
#     captures. So that address, on those days, needed neither a key nor a
#     proxy.
#   * Through the Scraping Browser API on 2026-09-18, a product page came
#     back as an Akamai `sec-cpt` BEHAVIOURAL interstitial — 4,383 bytes,
#     the real page nowhere in it, no widget and no sitekey anywhere on it.
#     It clears itself (it ships its own `location.reload(true)`), and the
#     engines wait it out; see page_flow.wait_out_self_clearing_challenge.
#     It is deliberately NOT sent to a solver: createTask validates almost
#     nothing and would charge for a task built from a page with nothing in
#     it to solve.
#
# What was NOT established: how this site treats a bare datacentre address
# with no proxy, or what a sustained high request rate from any of these
# paths does. Both are different experiments. Do not port the other family
# members' "datacentre addresses get refused" conclusion here without
# measuring it — it was true for them and is simply unmeasured for this
# site. See
